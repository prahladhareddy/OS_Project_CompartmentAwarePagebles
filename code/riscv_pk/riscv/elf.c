...
void load_elf(const char* fn, elf_info* info)
{
  ...
  for (int i = eh.e_phnum - 1; i >= 0; i--) {
    ...
    if(ph[i].p_type == PT_LOAD && ph[i].p_memsz) {
      ...
      if (__do_mmap(vaddr - prepad, ph[i].p_filesz + prepad, prot | PROT_WRITE, flags2, file, ph[i].p_offset - prepad, DEF_CID) != vaddr - prepad )
        goto fail;
      ...
      if (ph[i].p_memsz > mapped)
        if (__do_mmap(vaddr + mapped, ph[i].p_memsz - mapped, prot, flags|MAP_ANONYMOUS, 0, 0,DEF_CID) != vaddr + mapped )
          goto fail;
    }
  }
