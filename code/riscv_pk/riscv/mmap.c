...
typedef struct {
  ...
  unsigned refcnt;
  int prot;
  int cid;         // adding compartment id to metadata
} vmr_t;
...
static vmr_t* __vmr_alloc(uintptr_t addr, size_t length, file_t* file,
                          size_t offset, unsigned refcnt, int prot,int cid)
{
  if (!vmrs) {
...
uintptr_t clean(uintptr_t addr) // to neglect the compartment id bits 
{
  return (((addr)<<10)>>10);
}

uintptr_t put_cid(uintptr_t addr,int cid)  // for inserting compartment id into pagetable entry
{
  long x = cid;
  x = x<<56;            // I used last 8 bits to store compartment id 
  return clean(addr) | x;
}

int get_cid(uintptr_t addr) 
{
  return addr >>56;
}
...
{
  return clean(pte) >> PTE_PPN_SHIFT;
}
...
static pte_t* __attribute__((noinline)) __continue_walk_create(uintptr_t addr, pte_t* pte)
{
  ...
  *pte = put_cid(*pte,DEF_CID);      // mapping intermediate pages to default comapartment id (not usefull now as def_cid = 0 might be usefull when changing it)
...
static int __handle_page_fault(uintptr_t vaddr, int prot)
{
  ...
  else if (!(*pte & PTE_V))
  {
    ...
    *pte = put_cid(*pte , v->cid);     // adding compartment id present in meta data t page table entry
  }
...
uintptr_t __do_mmap(uintptr_t addr, size_t length, int prot, int flags, file_t* f, off_t offset,int cid) //
{
  ...
  vmr_t* v = __vmr_alloc(addr, length, f, offset, npage, prot, cid); //
  if (!v)
    return (uintptr_t)-1;
...
uintptr_t do_mmap(uintptr_t addr, size_t length, int prot, int flags, int fd, off_t offset,int cid) //
{
  ...
  spinlock_lock(&vm_lock);
    addr = __do_mmap(addr, length, prot, flags, f, offset , cid); //
...
uintptr_t __do_brk(size_t addr,int cid) //
{
  ...
  if (current.brk > newbrk_page)
    ...
    kassert(__do_mmap(current.brk, newbrk_page - current.brk, -1, MAP_FIXED|MAP_PRIVATE|MAP_ANONYMOUS, 0, 0, cid) == current.brk); //
  current.brk = newbrk_page;

  return newbrk;
}

uintptr_t do_brk(size_t addr,int cid) //
{
  spinlock_lock(&vm_lock);
    addr = __do_brk(addr,cid); //
  spinlock_unlock(&vm_lock);
  ...
  spinlock_lock(&vm_lock);
    for (uintptr_t a = addr; a < addr + length; a += RISCV_PGSIZE)
    {
      ...
      if (!(*pte & PTE_V)) {
        ...
        // to ensure no change in compartment id
        int cid = get_cid(*pte);
        *pte = put_cid ( pte_create(pte_ppn(*pte), prot_to_type(prot, 1)) ,cid);
      }
    }
  spinlock_unlock(&vm_lock);
  flush_tlb();
...
void __map_kernel_range(uintptr_t vaddr, uintptr_t paddr, size_t len, int prot)
{
  ...
  for (uintptr_t a = vaddr, i = 0; i < n; i++, a += RISCV_PGSIZE)
  {
    ...
    *pte = put_cid(*pte , DEF_CID);  // mapping with default comartment id (help full if we wanto change default compartment)
  }
}
...
uintptr_t pk_vm_init()
{
  ...
  size_t stack_bottom = __do_mmap(current.mmap_max - stack_size, stack_size, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_ANONYMOUS|MAP_FIXED, 0, 0,DEF_CID);
...
/*
this was my first nethodology i used

uintptr_t do_my_alloc(int a)
{
  if(a<0||a>255)
  {
    panic("PANIC :invalied compartment id\n");
  }
  long b=(long)a;
  uintptr_t paddr = __page_alloc();
  uintptr_t vaddr = current.brk;
  pte_t* pte = __walk_create(vaddr);
  *pte = pte_create( paddr >> RISCV_PGSHIFT, prot_to_type(-1 , 1));
  *pte = (*pte | (b<<56));
  current.brk += RISCV_PGSIZE;
  return current.brk - RISCV_PGSIZE;
}
*/
...
uintptr_t do_my_alloc(int a)
{
  ...
  uintptr_t vaddr = current.brk;
  vaddr = do_brk(vaddr+ RISCV_PGSIZE , a );
  return vaddr - RISCV_PGSIZE;
}
///////////////////////////////////////////////////////////////////////////
