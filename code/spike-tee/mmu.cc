...
int get_cid(reg_t pte)
{
  return pte>>56;
}

reg_t rem_pro(reg_t pte)
{
  return ((pte<<10)>>10);
}
...
reg_t mmu_t::walk(reg_t addr, access_type type, reg_t mode)
{
  ...
  for (int i = vm.levels - 1; i >= 0; i--) {
    ...
    reg_t ppn = rem_pro(pte )>> PTE_PPN_SHIFT;

    if(get_cid(pte) != proc->state.ucurrcap)
    {
      if(get_cid(pte) != 0 && get_cid(pte) != 1 )
      {
        printf("COMPARTMENTID MISSMATCH in compartment %d trying to access compartment %d\n",proc->state.ucurrcap,get_cid(pte) );
        abort();
      }
    }
